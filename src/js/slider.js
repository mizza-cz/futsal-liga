(function () {
                  ...
                  },
                  {
                    ...
                    value: function () {
                      var e = (this.config.rtl ? -1 : 1) * (this.drag.endX - this.drag.startX),
                        ...
                        n =
                          e < 0 && this.currentSlide + i > this.innerElements.length - this.perPage;
  ...
  const controls = document.querySelectorAll('.js-slider-control');
  const sliderNode = document.querySelector('.js-slider');
  const activeSlide = parseInt(sliderNode.dataset.active) || 0;

  const handleClickOnControl = (e) => {
    // prev?
    if (e.currentTarget.dataset.toggle === 'prev') {
      slider.prev();
      return;
    }
    // next
  ...
  const slider = new Siema({
    ...
    onInit: () => {
      sliderNode.style.opacity = 1;
    },
  });

  controls.forEach((control) => control.addEventListener('click', handleClickOnControl));
})();
